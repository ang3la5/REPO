...
const MovieDetailPage = () => {
  ...
  const poster = movie?.posterUrl?.includes('http')
    ? movie.posterUrl
    : `https://image.tmdb.org/t/p/w500${movie?.posterUrl}`;


  if (!movie) return <Typography sx={{ p: 4 }}>Loading...</Typography>;
  return (
        ...
        {movie && (
          <Card sx={{ backgroundColor: '#2a2a2a', color: 'white', mb: 4 }}>
            ...
            <CardContent>
              ...
              <Typography variant="body1" sx={{ mt: 2 }}>{movie.description}</Typography>
              <Typography variant="subtitle1" sx={{ mt: 1 }}>
                {t('genre')}: {movie.genre}
              </Typography>
              <Typography variant="subtitle1">
                {t('rating')}: {movie.rating?.toFixed(1) || 0} / 10
              </Typography>
